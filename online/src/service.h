...
public:  
    ...
    int send_buf(player_t* player, userid_t userid, uint16_t cmd, 
            const char* body, int bodylen, uint32_t ret = 0);
    int send_msg(player_t* player, userid_t userid, uint16_t cmd, 
            const google::protobuf::Message& message, uint32_t ret = 0);
    int send_cmsg(player_t* p, userid_t userid, uint16_t cmd, 
